...
DatesGrid.prototype.drawGrid = function(){
    ...
    var now = new Date(),
        nowStr = now.format("yyyy-M-d");
    for(var i = 0; i < this.helper.date_list.length; i++){
        var label = this.helper.date_list[i].getDate();
        var data_single = this.helper.date_list[i].format("yyyy-M-d");

        if(data_single === nowStr){
            label = "今天";
        }
    ...
    for(var i=0; i < 6; i++){
        grid.push("<tr>");
        for(var j=0; j < 7; j++){
            var labelContent = this.helper.meta_list[index] ? String(label_list[index]) : "";
            var lableStr = (labelContent != "") ? '<div class="day">'+ labelContent + '</div>' : "";
            var td = "<td" + meta_list[index] + name_list[index] + data_list[index] + ">" + lableStr + "</td>";
...
DatesGrid.prototype.drawTitle = function(){
    ...
    for(var i=0; i<7; i++){
        var className = (i === 0 || i ===6) ? "td-date-title font-red" : "td-date-title";
        var td = "<td class='" + className +"'>" + title_label_list[i] + "</td>";
        title.push(td);
    }
    title.push("</tr></thead>");
