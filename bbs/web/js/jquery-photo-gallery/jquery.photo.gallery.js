...
//图片查看器
	$.fn.extend({
		...
		photoGallery: function(options) {
			...
			var $parent = $("#J_pg", window.parent.document),
				x = 0,
				y = 0,
				l = 0,
				t = 0,
			...
			$gallery.on("mouseover",function(e){
				...
				y=dragY;
				//获取左部和顶部的偏移量
				l = $parent[0].offsetLeft;
				t = $parent[0].offsetTop;
				//开关打开
				isDown = true;
				e.stopPropagation();
			}).on("mousemove",function(e){
				if(dragX > 0){
					//console.log("value==" + $parent.css("left"))
					var isMoveParent = false;
					var nextDragX=e.pageX || e.clientX;
					var nextDragY=e.pageY || e.clientY ;
					//console.log("nextDragX="+nextDragX)
					var o = $image.offset(),
					...
					if(isMoveParent){
						...
						//计算移动后的左偏移量和顶部的偏移量
						var nl = nextDragX - (x - l);
						var nt = nextDragY - (y - t);
						$parent.css({ left:  nl + 'px', top: nt + 'px'});
						l = $parent[0].offsetLeft;
						t = $parent[0].offsetTop;
						return;
					}
...
$.extend({
	...
	openPhotoGallery : function(obj){
		...
		$("#J_pg").remove();
        $("<div id='J_pg_div' style = 'position:absolute;z-index: 99999'></div>").appendTo("body");
		$("<iframe></iframe>").appendTo("#J_pg_div")
			.attr("id", "J_pg")
